...
bf = agent(m, footprints=True, color=COLOR.red, filled=True)
mainPath = agent(m, footprints=True, color=COLOR.yellow,filled=True)
...
if algorithm == "DFS": ##  DFS
    ...
    if searchPath : m.tracePath({bf:BFS_exploredPath}, delay=40) #if the whole path needed
    m.tracePath({a:BFS_fwdpath},delay=130)
...
m._win.attributes('-topmost', True)
m._win.update()
m._win.attributes('-topmost', False)
m._win.lift()
m._win.focus_force()
m.run()